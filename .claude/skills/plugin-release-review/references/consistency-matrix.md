...
| Scripts derived from another plugin's | the plugin's own end-to-end suite (installer commands in a sandbox) | A function lost in the port prints `command not found` in Bash and the command can still exit 0; the suite must fail on that text, not only on exit codes |
...
| Optional components that cost money or need a plan (workflows, many agents) | README Requirements, Security Process row, Other components | The requirement (plan, `/config` toggle) is a Requirements row, and the cost sits in the Process row; the Security table keeps only the template's rows (Read, Write, Process, Network, Credentials) |
