...
- [ ] `make versions` and `make validate-cli` (`claude plugin validate --strict`) pass locally.
- [ ] The plugin README follows the [README contract](../templates/plugin-README-reusable-template.md): all 15 sections present, and **What it does not do**, **Security**, and **Limitations** match this version (`make validate` checks the structure).
- [ ] The root [README catalog](../README.md#-plugin-catalog) has this plugin's row, matching its README (`make validate` enforces it).
...
<!-- Commands you ran and what you observed. `make check` is the CI gate for every PR. -->
