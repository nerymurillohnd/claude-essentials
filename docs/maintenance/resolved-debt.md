...
- **Positive verification:** `make check` exits 0 (2026-09-22); the extractor suite passes 102 of 102 and the hook suite 22 of 22 on bash 3.2 and 5.x with Python 3.14, and `/usr/bin/python3` 3.9.6 gets `<script> needs Python 3.14 or later; this is python3 3.9.6 …` and exit 5 from all seven extractors.
