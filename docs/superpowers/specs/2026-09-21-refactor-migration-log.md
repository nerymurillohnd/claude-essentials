...
### `post-edit.sh` becomes reminder-only (maintainer's decision, 2026-09-22)

- Instruction: "elimina de ese hook y su script todo lo que ejecuta para lint/format/typecheck con shellcheck, shfmt, ruff y basedpyright." Removed from `post-edit.sh`: the shell branch (`shfmt -w`, `shellcheck -x`) and the text branch (`make -s fix-file`: Ruff, basedpyright, canonical JSON, text bytes), with their block output. Kept: the once-per-session plugin version reminder and the Bash-stamp file discovery that feeds it. The hook stays wired in `.claude/settings.json`.
- What is lost is the immediate feedback only; coverage is unchanged: `guard-commit.sh` denies a commit that fails `make lint-staged`, and `make check` runs everything, locally and in CI.
- Verified: `shellcheck` and `shfmt -d` clean; `bash -n` under bash 5 and `/bin/bash` 3.2; a misformatted `.sh` written through a Write payload came back byte-identical with no output; an edit to `plugins/block-no-verify/skills/block-no-verify/SKILL.md` still printed `plugins/block-no-verify has runtime changes since block-no-verify--v0.1.2 (first: skills/block-no-verify/SKILL.md)…` (file restored afterwards).
- `scripts/harness/test_post_edit.py` rewritten: `test_an_edit_is_never_rewritten_or_blocked` (shell, Python, JSON × both bashes), `test_a_runtime_edit_reminds_once_per_session`, `test_an_exempt_edit_does_not_remind`, `test_a_file_outside_the_tree_is_skipped`, `test_the_hook_calls_no_lint_format_or_type_tool` → 13 passed; Ruff and basedpyright `all` clean. The reminder had no test before this change.
- Also corrected: `make fix-file` help text, and the docstrings of `scripts/lint/shell_files.py` and `scripts/hygiene/test_tooling_alignment.py` that described the old hook.

### shell-quality 0.2.0 ships in this PR (maintainer's decision, 2026-09-22)

- Decision: "No, no, no, no es ningún follow up. Por eso te pedí que no hicieras el push. Aplica las correcciones ahorita." The PR's label changes from `bump: none` to `bump: minor`; `make -s versions` → `shell-quality 0.2.0 runtime: skills/shell-hooks/SKILL.md → bumped from 0.1.1 (minor)`, `Computed label: bump: minor`.
- Design: `/plugin-design shell-quality`, spec `docs/superpowers/specs/2026-09-22-shell-quality-design.md`, checklist 10/10 with approval "Aprobado, construye"; evals authorized as a pilot only ("Solo piloto $5").
- Built: `references/audit.md` (the audit Claude performs, from `man shellcheck` 0.11.0 RC FILES/ENVIRONMENT and `man shfmt` 3.14.1, including a live latest-release check — ShellCheck v0.11.0 and shfmt v3.14.1 on 2026-09-22 via the GitHub MCP), the `SKILL.md` workflow (audit → report and recommend → choose the configuration → `apply-config` → scope → preflight → install), `config-modes.md` (scenario → mode), `rollback.md` (undoing `apply-config`), `manage.sh` (`apply-config`, per-level `assess` inventory, `preflight` message), the handler's guard (denies `apply-config`; version marker 0.2.0), eval cases `05-audits-before-proposing` and `06-offers-choice-with-existing-config` with scaffolds, `evals.yml` passing `--scaffold`.
- Verified live in sandboxed repositories: inventory reports a parent-folder rc as governing and the user rc as unused, `SHELLCHECK_OPTS` reported; a draft that does not parse is refused with ShellCheck's `SC1134` and nothing written; unknown `enable=` warned; adopting the recommended rc over an existing one backs it up under `.git/shell-quality-backups/`; `--rc-location user` writes the XDG rc with a backup under `~/.claude/backups/shell-quality/`; repeat → `unchanged`; usage errors → 2.
- Suites under `bash` 5.3 and `/bin/bash` 3.2: `test-manage.sh` 66 passed, `test-gate.sh` 83 passed (includes "Claude running apply-config is denied"). ShellCheck and shfmt clean on every changed script, no directive added.
- `make validate` → `M1-M10 pass`, plugins pass (DEBT-0029 warning only); `claude plugin validate plugins/shell-quality --strict` → passed; frontmatter 1,193 chars, no `": "`.

### Every suppression removed from the code and the configuration (maintainer's decision, 2026-09-22)

- Instruction: remove the suppressions an inventory found, after verifying each. Resolves the **recorded defect** in deviation 9: the three file-wide `# shellcheck disable=SC2016` are gone, and so is the exemption that let them in.
- ShellCheck: all 22 `# shellcheck disable=` directives in 11 scripts removed by changing the code, none by moving them. Two (`checklist-gate.sh`) suppressed nothing, since ShellCheck already exempts literals passed to `jq`. Literals holding `$` or backticks for jq, the hook shell or Markdown are double-quoted with those characters escaped (values proven identical, e.g. `gate.sh`'s template hashes the same); `checklist.sh` calls jq directly; `session-start.sh` builds the code spans in a bounded loop; `STYLE_FLAGS` is an array; block-no-verify's `main` ends in `return 0`, because ShellCheck 0.11 reports SC2329 on a trap-only function whenever the script ends in `exit` (reproduced on a four-line script). The 41 masked returns in verify-completion's suite are restructured, and a Stop payload jq cannot build now fails its case. The two `source=/dev/null` in Python-embedded drivers were dead text (ShellCheck never reads them) and are removed.
- Ruff: 18 of the 21 ignores removed. Re-enabling each, one at a time, raised zero findings and no `ruff format` conflict warning; only `COM812` (229 findings, the one conflict `ruff format` reports) and `S603` (18, documented upstream as prone to false positives) stay global. `S101` moves to `per-file-ignores` for `scripts/**/test_*.py` and `conftest.py`, so an `assert` outside the tests is now a finding.
- basedpyright: `reportImplicitStringConcatenation = false` removed. The recorded reason (the same exception as `ISC002`) was wrong: the check accepts a concatenation in its own parentheses, which `ISC003` also accepts. Its 27 sites are parenthesized; the five that were a call's only argument became a named value, because `UP034` rejects a second pair of parentheses there.
- Gates: `Q1` now pins the two-rule ignore list, the one `per-file-ignores` map and zero `report* = false`. `Q3` parses every shell script with `shfmt --to-json`, plugin suites included (the exemption is removed), and fails on any real `disable=` or `source=/dev/null` comment, while `shell-quality`'s fixtures inside strings stay data; embedded shell in Python is covered by two new patterns. Both directions checked: an injected directive in `gate.sh` and one in `test_repo_root.py` each failed the gate, and the restored tree passed.
- Versions: block-no-verify 0.1.3 and verify-completion 0.1.2 (patch, handler stamp 0.1.3), shell-quality's unreleased 0.2.0 entry extended, ruff-quality's change recorded under `[Unreleased]` because that plugin's version is pending other work on this branch (`make -s versions` still reports its V1).
- `make check` → rc 0 (`903 passed, 1 skipped` fast, `269 passed` slow, every plugin suite PASS under bash 5.3 and `/bin/bash` 3.2; verify-completion 125 cases before and after).
