...
| completion verifier: edges | hostile and odd inputs in both directions | about 30 rows failing | Fixed: tracebacks with an undocumented exit 1, a DTD accepted in UTF-16, text inserted then deleted shown as original, moves shown twice, an unterminated CSV quote swallowing rows, `²` accepted as a digit, `-`-prefixed file names read as options, control characters in the gate's JSON. After `repo-auditor` on `31e9e61`: `--scale` below 1 exits 2 (suite case); a corrupt image that every converter rejects keeps exit 5, now documented, because it cannot be told apart from a missing decoder and the message quotes each converter's error; gate time above ~12 MB is a README Limitations row |
...
Final suites on the branch: `test-scripts.sh` 102/102 and `test-hooks.sh` 22/22 under `bash`
