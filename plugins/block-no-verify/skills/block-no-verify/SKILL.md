...
description: This skill should be used when the user asks to "block --no-verify", "stop Claude from skipping my pre-commit hooks", says "Claude keeps using --no-verify / HUSKY=0 / git commit -n", asks to "enforce commit signing", "protect my Git hooks", "install the no-verify guard", "is the no-verify policy installed", or "uninstall block-no-verify", and when a task involves committing, pushing, merging, rebasing, Git hooks (pre-commit, husky, lefthook, core.hooksPath), or commit/tag signing. It assesses the repository, recommends a settings scope, and only after an explicit scope choice installs a PreToolUse hook that denies Git commands Claude runs to bypass local verification or signing. Do not use it for read-only Git questions (status, log, diff, show, blame, reflog, branch listing) or for explaining Git concepts.
