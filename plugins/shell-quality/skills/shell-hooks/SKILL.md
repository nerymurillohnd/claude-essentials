---
name: shell-hooks
description: This skill should be used when the user asks to "add a ShellCheck hook", "run shfmt and ShellCheck after every edit", "make Claude lint shell scripts automatically", "set up a shell lint gate / after-edit hook / Stop hook", "stop Claude from adding shellcheck disable", "don't let Claude finish with ShellCheck errors", "install / check / update / remove the shell-quality hook", or asks which ShellCheck or shfmt configuration such a hook should use. It assesses the project, explains the three configuration modes with the exact configuration each one applies, and only after the user chooses a scope and a mode installs Claude Code hooks that format and check every shell script Claude edits, deny suppression directives and configuration changes, and block the end of the turn until everything is clean. It never installs anything without that explicit choice. For using ShellCheck and shfmt themselves (codes, configuration, migration, CI), use the shell-lint skill.
