...
| --- | --- | --- | --- | --- |
| `project` | `<repo>/.claude/settings.json` | `<repo>/.claude/hooks/shell-quality-gate.sh` | Everyone who runs Claude Code in this repository, after they pull | Yes: commit the settings, the handler, and a new `.shellcheckrc` / `.editorconfig` change together |
| `local` | `<repo>/.claude/settings.local.json` | `<repo>/.claude/hooks/shell-quality-gate.sh` | This user, this repository | No: the handler, `settings.local.json`, and a written `.shellcheckrc` are added to `.git/info/exclude` unless already ignored or tracked |
| `user` | `~/.claude/settings.json` | `~/.claude/hooks/shell-quality-gate.sh` | This user, every project on this machine | No |
...
- **project** — the team wants every Claude Code session in the repository to
  meet the same bar. Teammates need bash, jq, shellcheck, and shfmt; a committed hook whose
...
- **user** — one person wants the gate in every project with shell scripts. Cloud sessions
...
`bash "$CLAUDE_PROJECT_DIR/.claude/hooks/shell-quality-gate.sh"`; for user
scope it is `bash "$HOME/.claude/hooks/shell-quality-gate.sh"`. `A` is
`--config-mode <mode> --max-blocks <n>`, plus `--rcfile "<path>"` when pinned and `--style-fallback` for user scope with `recommended`.
...
- The `Bash` post group has no status message, so ordinary shell commands do
  not flash a spinner; the handler exits at once when no shell script changed.
- Exact-match matchers (`Write|Edit`) never match other tools.
...
2. Copy `assets/shell-quality-gate.sh` byte for byte to the handler path
   (`cp`, then `cmp`).
3. For `recommended`, copy `assets/shellcheckrc` to the path in
   [config-modes.md](config-modes.md) only if no rc file exists there, and
   append `assets/editorconfig-shell` to `.editorconfig` only if it sets no
   shell style.
