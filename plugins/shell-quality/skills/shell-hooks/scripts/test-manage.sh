#!/usr/bin/env bash
# End-to-end suite for the shell-quality manage.sh: every command in a throwaway project, user
...
#   the bash that runs manage.sh. Needs bash, jq, git, shellcheck >= 0.11, and shfmt >= 3.12.
...
profile="${here}/../assets/shellcheckrc"
...
groups_in() { jq -r '[.hooks[]? | .[] | select(any(.hooks[]; .command | test("shell-quality-gate")))] | length' "$1" 2>/dev/null; }

m assess
check "assess exits 0" rc_is 0
check "assess has no shell errors" no_shell_errors
check "assess reports shellcheck and shfmt" has "shfmt: "
check "assess reports a baseline for each mode" has "defaults (--norc"
...
check "show-config recommended prints the rc profile" has "external-sources=true"
check "show-config recommended prints the shfmt style" has "[[shell]]"
...
check "install wrote the recommended profile" cmp -s "${profile}" "${proj}/.shellcheckrc"
check "install appended the shfmt style" grep -q "\[\[shell\]\]" "${proj}/.editorconfig"
check "install copied the handler" test -x "${proj}/.claude/hooks/shell-quality-gate.sh"
...
check "preflight enforces the shellcheck minimum" has "ok   shellcheck "
check "preflight enforces the shfmt minimum" has "ok   shfmt "
...
check "uninstall removed the handler" test ! -e "${proj}/.claude/hooks/shell-quality-gate.sh"
check "uninstall removed the emptied settings file" test ! -e "${proj}/.claude/settings.json"
check "uninstall kept the profile" test -f "${proj}/.shellcheckrc"
...
m install --scope local --config-mode own --config-path "${proj}/.shellcheckrc"
check "install (local, own, pinned) exits 0" rc_is 0
check "the pinned path is project-relative" has "\"\$CLAUDE_PROJECT_DIR/.shellcheckrc\""
...
left=$(grep -c 'shell-quality' "${proj}/.git/info/exclude" || true)
...
check "user scope writes under CLAUDE_CONFIG_DIR" test -x "${CLAUDE_CONFIG_DIR}/hooks/shell-quality-gate.sh"
