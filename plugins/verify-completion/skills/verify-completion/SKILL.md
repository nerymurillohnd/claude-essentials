...
description: Prove the work is really finished before telling the user so, and never let an unbacked claim stand as done — run six evidence gates (adversarial review of the real diff, outcome, counterpart, distrusting green checks, both directions, evidence left behind), record each as PASS, FAIL, N/A or BLOCKED, and close with a Verification record. Re-read the diff and re-run the commands instead of trusting an earlier run or a subagent report, and scale an independent read-only verifier to the size of the change. A failing gate restarts the sequence from the first. A passing record never authorizes a commit, push, publish or deploy.
when_to_use: Right before a reply says or implies that work is complete, fixed, working, verified or ready to commit, open a PR, review, hand off or deploy; when the user asks whether it is really done, in English or Spanish; and when the verify-completion Stop hook reports a claim with no valid record. Not for intermediate steps, plans or progress updates.
---
