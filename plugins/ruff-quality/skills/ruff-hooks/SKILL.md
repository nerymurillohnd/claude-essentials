---
name: ruff-hooks
description: This skill should be used when the user asks to "add a Ruff hook", "lint and format Python after every edit", "make Claude run Ruff automatically", "set up a Ruff gate / after-edit hook / Stop hook", "stop Claude from adding noqa", "don't let Claude finish with Ruff errors", "install / check / update / remove the ruff-quality hook", or asks which Ruff configuration such a hook should use. It assesses the project, explains the three configuration modes with the exact configuration each one applies, and only after the user chooses a scope and a mode installs Claude Code hooks that fix, format, and lint every Python file Claude edits, deny suppression comments and configuration changes, and block the end of the turn until everything is clean. It never installs anything without that explicit choice. For using Ruff itself (commands, rules, migration, CI), use the ruff skill.
