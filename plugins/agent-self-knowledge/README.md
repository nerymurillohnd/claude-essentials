...
[![Python](https://img.shields.io/badge/Python-3.7%2B-3776AB?logo=python&logoColor=white)](#-requirements)
...
> and `/claude-code-docs` appears in the slash-command menu.
...
No MCP servers and no credentials. The skill's `allowed-tools` grants its own
bundled script plus read-only `curl` against `code.claude.com` and
`raw.githubusercontent.com/anthropics/*`, so retrieval runs without a prompt for
each command. Network destinations are listed under [Security](#-security).
...
| --- | --- | --- | --- |
| Claude Code | ⚠️ Partial | 2026-09-20, Claude Code 2.1.278 | Retrieval exercised in a real session on macOS; Linux and WSL follow from stdlib Python but are unverified. Windows without Git Bash is unsupported: the grant is a `Bash` rule. The consumer smoke test and the evals have not been run. |
...
| Process | `python3` running the plugin's own bundled script. |
